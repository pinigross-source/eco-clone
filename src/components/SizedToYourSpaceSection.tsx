          ...
          {products.map((p) => (
            <ScrollReveal key={p.name} variant="fadeUp">
              <article className="group rounded-2xl border border-foreground/10 bg-card overflow-hidden hover:border-foreground/20 transition-colors h-full flex flex-col relative">
                ...
                <div className="relative w-full aspect-[4/3] min-h-[260px] overflow-hidden bg-card">
                  <img
                    src={p.imageSrc}
                    alt={p.imageAlt}
                    loading="lazy"
                    decoding="async"
                    className="absolute inset-0 h-full w-full object-contain group-hover:scale-105 transition-transform duration-700"
                  />
                </div>
