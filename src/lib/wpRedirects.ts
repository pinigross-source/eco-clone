...
export const REDIRECT_MAP: Record<string, string> = {
  ...
  "/how-indoor-": "/blog",
  "/supporting-immune-health": "/health-benefits",
  "/product-registration-form": "/product-registration",
  ...
  "/odor-elimination": "/health-benefits#odor-elimination",
  "/mold-5": "/health-benefits#mold-prevention",
...
export const PATTERN_REDIRECTS: Array<{ pattern: RegExp; target: string }> = [
...
export function resolveWpRedirect(pathname: string): string | null {
  ...
  if (REDIRECT_MAP[lower]) return REDIRECT_MAP[lower];
  for (const { pattern, target } of PATTERN_REDIRECTS) {
    const match = path.match(pattern);
    if (match) {
      let resolved = target;
      if (match[1]) resolved = resolved.replace("$1", match[1]);
      if (match[2]) resolved = resolved.replace("$2", match[2]);
      return resolved;
    }
  }
  return null;
